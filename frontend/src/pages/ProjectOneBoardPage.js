...
useEffect(() => {
  ...
  const fetchBoardInfo = async () => {
    try {
      const boardResponse = await fetch(`http://127.0.0.1:8000/api/projects/project/${projectId}/teamboard/${boardId}/`, {
        method: 'GET',
        headers: {
  ...
  const fetchTasks = async () => {
    try {
      const tasksResponse = await fetch(`http://127.0.0.1:8000/api/projects/project/${projectId}/teamboard/${boardId}/teamtask`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization':'Bearer ' + String(authTokens.access)
...
const handleDelete = async () => {
  try {
    const response = await fetch(`http://127.0.0.1:8000/api/projects/project/${projectId}/teamboard/${boardId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + String(authTokens.access)
